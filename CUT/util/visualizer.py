...
def save_images_batch(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    """Save images to the disk.
    ...
    # short_path = ntpath.basename(image_path[0])
    # name = os.path.splitext(short_path)[0]
    short_path_list = [ntpath.basename(single_image_path) for single_image_path in image_path]
    name_list = [os.path.splitext(short_path)[0] for short_path in short_path_list]

    # webpage.add_header(name)
    ims, txts, links = [], [], []

    for label, im_data in visuals.items():
        im = util.tensor2im_batch(im_data)
        for i in range(len(image_path)):
            name = name_list[i]
            image_name = '%s/%s.png' % (label, name)
            os.makedirs(os.path.join(image_dir, label), exist_ok=True)
            save_path = os.path.join(image_dir, image_name)
            util.save_image(im[i], save_path, aspect_ratio=aspect_ratio)
        ims.append(image_name)
        txts.append(label)
        links.append(image_name)
    webpage.add_images(ims, txts, links, width=width)
