...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif netG == 'resnet_9blocks_lfeb':
        net = ResnetGenerator_LFEB(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
...
class ResnetGenerator_LFEB(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        assert(n_blocks >= 0)
        super(ResnetGenerator_LFEB, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        model = [nn.ReflectionPad2d(3),
                 ...
                 LFEB(inchannel=input_nc, mksize=13, bksize=-1, blur=True, use_arg=False, scale=False, add=True, conv3=False,
                      mode=0, norm=True, angle0=30, dilation=2, mid=False),
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
        ...
        model += [nn.ReflectionPad2d(3)]
        model += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0),
                  LFEB(inchannel=output_nc, mksize=13, bksize=-1, blur=True, use_arg=False, scale=False, add=True,
                       conv3=False,
                       mode=0, norm=True, angle0=30, dilation=2, mid=False),
...
class ResnetBlockLFEB(nn.Module):
    """Define a Resnet block"""
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        Original Resnet paper: https://arxiv.org/pdf/1512.03385.pdf
        """
        super(ResnetBlockLFEB, self).__init__()
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias)
