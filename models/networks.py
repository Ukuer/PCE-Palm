...
def define_G(input_nc, output_nc, nz, ngf, netG='unet_128', norm='batch', nl='relu',
    ...
    if netG == 'unet_128' and where_add == 'input':
        ...
        net = G_Unet_add_all(input_nc, output_nc, nz, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                             use_dropout=use_dropout, upsample=upsample)
    elif netG == 'unet_256_cycle':
        net = G_Unet_add_all_cycle(input_nc, output_nc, 0, 8, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                             use_dropout=use_dropout, upsample=upsample)
        print("定义cycle loss 的生成器")
    else:
...
class G_Unet_add_all(nn.Module):
    ...
    def forward(self, x, z):
        ...
        #z2 = torch.randn(z.shape[0], z.shape[1]).to(z.device)
        z = self.mappnet(z)
        z = z.unsqueeze(1).repeat([1, 8, 1])
        # z2 = self.mappnet(z2)
        # z2 = z2.unsqueeze(1).repeat([1, 8, 1])
        #
...
class UnetBlock_with_z(nn.Module):
    ...
    def forward(self, x, z_all,z_idx):
        ...
        #z_img = z2.view(z2.size(0), z2.size(1), 1, 1).expand(z2.size(0), z2.size(1), x.size(2), x.size(3))
        #x_and_z = torch.cat([x, z_img], 1)
        z = z_all[:,z_idx]
        #print("z_idx=",z_idx)
        #print("z=",z[0,1])
...
class G_Unet_add_all_cycle(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
                 norm_layer=None, nl_layer=None, use_dropout=False, upsample='basic'):
        super(G_Unet_add_all_cycle, self).__init__()
        nz=0
        self.nz = 0
        # construct unet structure
        unet_block = UnetBlock_with_z_cycle(ngf * 8, ngf * 8, ngf * 8, nz, None, innermost=True,
                                      norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
        unet_block = UnetBlock_with_z_cycle(ngf * 8, ngf * 8, ngf * 8, nz, unet_block,
                                      norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout, upsample=upsample)
        for i in range(num_downs - 6):
            unet_block = UnetBlock_with_z_cycle(ngf * 8, ngf * 8, ngf * 8, nz, unet_block,
                                          norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout, upsample=upsample)
        unet_block = UnetBlock_with_z_cycle(ngf * 4, ngf * 4, ngf * 8, nz, unet_block,
                                      norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
        unet_block = UnetBlock_with_z_cycle(ngf * 2, ngf * 2, ngf * 4, nz, unet_block,
                                      norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
        unet_block = UnetBlock_with_z_cycle(
            ngf, ngf, ngf * 2, nz, unet_block, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
        unet_block = UnetBlock_with_z_cycle(input_nc, output_nc, ngf, nz, unet_block,
                                      outermost=True, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
        self.model = unet_block
...
class UnetBlock_with_z_cycle(nn.Module):
    def __init__(self, input_nc, outer_nc, inner_nc, nz=0,
                 submodule=None, outermost=False, innermost=False,
                 norm_layer=None, nl_layer=None, use_dropout=False, upsample='basic', padding_type='zero'):
        super(UnetBlock_with_z_cycle, self).__init__()
        p = 0
        downconv = []
        if padding_type == 'reflect':
            downconv += [nn.ReflectionPad2d(1)]
